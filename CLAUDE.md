...
- **The dashboard is a frame plus a request per card, and `DASHBOARD_SECTIONS` is the list.** `/api/dashboard` returns the period, the filters, the KPI strip and `sections` — the *names* of the cards that hang on it; `/api/dashboard/section/{name}` serves one card. It used to return all eight aggregates in one response, and the page waited on the slowest before drawing anything. The business map met this exact problem first ("five layers in one call waited 7.5 s on the PostgreSQL deployment before the first could paint") and answered it the same way, so the two surfaces now solve it once between them: `useDashboardSections` fans out with `useQueries` and a card paints when *its* answer arrives. **Splitting rather than parallelising inside the endpoint is deliberate** — `DB_POOL_SIZE` is 5 with 5 of overflow, so one request running eight aggregates at once would hold eight connections and two readers would exhaust the pool; a request per card holds one. The registry is what the frame publishes *and* what the section route refuses against, so a card cannot be added in one place and forgotten in the other — the rule at the top of this file applied to a route. The browser takes the names from the frame rather than carrying its own list, so a card added on the server is fetched without the browser being taught about it. **Only the frame audits**: opening the dashboard is one act however many requests draw it. One real cost was accepted rather than hidden — on This Year and Last Year the Sales Trend and Monthly Performance windows coincide and the same query runs twice, which was deduplicated while they shared a request and cannot be now; it costs a second aggregate rather than a second wait, because the two requests are in flight together. **Measured through the running service, warm and over `127.0.0.1`**: first figures reach the screen in 0.39s (This Month) and 1.77s (This Year), and the whole page completes in 1.90s and 4.57s — against 2.47s and 6.82s for the same six cards fetched one at a time, so the fan-out is ahead on both periods and not only on the long one. An earlier draft of this paragraph claimed the opposite — that on a short period one request would finish sooner — on the strength of an **in-process** measurement that called the builders directly against PostgreSQL. That approximation understated the service's own parallelism by about two and a half times, putting this same page at 4.66s where uvicorn serves it in 1.90s, because it neither inherits the service's warm pool nor schedules the way its threadpool does. **Compare request shapes through the service; compare queries in process.** The other trap in the same measurement is worse because it is silent: `localhost:8000` resolves to `::1` first and the service binds IPv4 only, so every request through that name pays a flat **2.02s** connect timeout before falling back — `/health` times at 0.007s over `127.0.0.1` and 2.019s over `localhost`, and a shape that makes seven requests wears that tax seven times while the one it replaced wore it once.
- **Growth is the same date range one year earlier, and a resolved range carries two comparison pairs so the two questions cannot be confused.** `ResolvedDateRange.compare_from` / `compare_to` is the **preceding period** — the month before, the quarter before — and answers "compare August with July", which the assistant is genuinely asked. `growth_from` / `growth_to` is **the same dates a year earlier**, and is what every surface drawing a figure it calls *growth* must read. Bangladeshi FMCG is seasonal: the month before is not a baseline for the month after. Both are computed once by `DateResolver._build`, because the resolver already owns period semantics and a rule that lives anywhere else is a rule some surface cannot reach. The end is capped at today before the shift (`_elapsed`), so a window running into the future — "This Year" reaches next June — is compared by the part of it that has happened rather than against a complete previous year. **There was one pair for a long time, and that is precisely how this broke**: growth was the only thing anyone wanted a comparison for, the pair meant the preceding period, and three surfaces reached for the only pair there. The Region Performance card reported **+96%** for Region-Chattogram in August 2026 — 1.49 Cr against July's 0.76 Cr rather than August 2025's 1.68 Cr — and **contradicted the bars drawn beside it**, which showed the decline plainly under the same legend. A chart disagreeing with itself is what makes this a defect rather than a preference. Fixed globally in one pass rather than card by card: the dashboard KPI and region card, the **business map**'s growth metric (a map shaded by growth read a seasonal rise as a surge, and the same region came out green there and red on the dashboard), the **Materials page** breakdown and its Top/Bottom 10, the **Sales page** headline, and `reporting.service._period_metrics`, which grew MTD against the previous *month* and now grows it against the same month-to-date a year earlier — returning `previous_year_mtd` beside `previous_month` so each figure names its own base. **A label moves with its figure**: `common.vsPrevious` was "vs previous period" and is now "vs same period last year" in both languages, because changing the number and leaving the caption is worse than the bug. Four things deliberately still read the preceding period or neither pair, and must: `get_sales_trend`'s first-against-last point (it asks whether the line is rising, not versus when), and `compare_totals` / `target_vs_actual` / `_variance_by` / `RootCauseToolInput`, which are the *machinery* — the caller states both windows, and root cause is explicitly "variance of one period against another". The fault was never in the arithmetic; it was in which window each surface handed it.
- **The dashboard's region card is one card, and `region_overview` is deliberately not `region_performance`.** Region Performance and Target Achievement were two cards side by side asking one question: the first drew each region's net sales, and the second already carried that same figure as `actual_sales`, read a second time from the same view over the same window. Two reads of one number is how two cards on one screen come to disagree, and it cost a second pass over the fact view for a column the other call already had — so the dashboard now makes one `get_target_achievement` call and serves it as the **`region_overview`** card. The key is renamed rather than reused because `region_performance` still means the sales-only tool's result on `/api/pages/sales`, which is a different shape; one word over two shapes is the stale-name failure the top of this file opens with. `ComboBarLineChart` draws it: four bars on a left BDT axis and two lines on a right percentage axis (achievement, growth), because an achievement of 75 shares a money axis with 2,000,000 only by becoming invisible. **The bars are the Monthly Performance card's four** — two earlier years, the plan, the outcome — reached by `AchievementToolInput.compare_years`, which is this window shifted back by whole years exactly as `TrendToolInput.compare_years` is, and asked for on the dashboard's call alone so the Target page and the assistant carry no such key. A year that recorded nothing is **dropped rather than drawn along the floor**, and the surviving years are named in `chart.series` with the platform's own period names, so one browser helper draws both cards. The year-earlier actual drives the growth line and is **also** the first earlier-year bar, which is the point: `previous_sales` and `net_sales_minus_1` are now the same figure by construction, so the line and the bar are arithmetically incapable of disagreeing. They were two different measures under one legend, and that is what produced the +96%. `AchievementToolInput.compare_from` / `compare_to` are still **optional as a pair**, validated together, and the tool still returns neither `previous_sales` nor `growth_percent` **at all** without them rather than present-and-null, so the assistant's achievement tables do not grow two columns of dashes; a region absent from the comparison window keeps `None` for both rather than 0.0 and −100%. The dashboard now always passes them, taking `growth_from` / `growth_to` from the resolved range: a year-shifted window can never coincide with the window it is compared against, so the case the old conditional guarded — a comparison bar equal to the actual, a growth line flat at 0% — cannot arise. The KPI beside the card reads the same pair, so the card and the headline cannot tell different stories about one period; `test_platform_api` pins them equal. **The one place the line and the bars still describe different spans is "This Year"**, where the line compares elapsed with elapsed while `A 25-26` is a whole financial year — inherent to an unfinished year, since cutting the bar would make it no longer the year its label names.
- **The Monthly Performance card is a year of months over its own window, and the two percentages are deliberately absent from `chart.series`.** It began as a second drawing of `sales_trend` — one request, two cards — and that held only while the reader's period was long enough to be charted by month. On the default "This month" the trend is charted by day, so the card drew one bar per **date**, with no target and no earlier year to set it against, under a title promising months. It now makes its own `get_sales_trend` call at month granularity over **the financial year the selected period ends in**, returned as `monthly_performance`; `sales_trend` is untouched and still follows the period exactly, because a short period charted by day is what that line chart is *for*. Widening the shared query was the alternative and was rejected for changing a card that was not the subject. The cost is real and is named rather than hidden: two reads of one view can disagree, and these two deliberately do, because they cover different windows — so the card states which financial year it is showing, and every other filter still narrows it. `_multi_year_trend` derives `achievement_percent` (net sales against target) and `growth_percent` (against `net_sales_minus_1`) onto each row — **no business calculation lives in the browser** — and puts **neither into `chart.series`**. That list is what `trendSeriesFrom` turns into lines on the *taka* axis, and it is read by the Sales Trend card and by `/api/pages/sales`'s `monthly_trend`, neither of which was being changed; an achievement of 93 plotted there sits on the floor beside figures in the crores. The combo card names its own lines instead, and `barSeriesFrom` orders the bars history → plan → outcome while reusing `trendSeriesFrom`'s colour rule, so a year keeps the same hue on both cards. **Both keys are gated on the series being drawn, not on the argument being passed**: a caller can ask for a target no month of the period has, and a column of nothing but nulls is the empty column the gate exists to prevent — which is also why a dropped comparison year takes its growth column with it. A month with no target has no achievement and a month the prior year did not record has no growth; neither is 0% or −100%. Every tool is scoped, so `_narrower_than_the_country` adds a note naming the reader's data scope and their active filters whenever the figures are not the country's — **narrowed and labelled, never hidden**, because withholding the card would deny a regional manager the one view of their own year. It was titled "Monthly Country Performance" until the word did more harm than good on a card a regional manager opens; the note is what carries the distinction now, and it does so whatever the title says.
...
- **A caller that draws no invoice count does not pay for one, and `COUNT(DISTINCT invoice_no)` is the most expensive thing on the sales path.** It is not a column like the others: a distinct count forces a `GroupAggregate` — every row sorted by the group columns and then the invoice — where the plain sums hash-aggregate and never sort at all. Measured on the deployment, three financial-year windows grouped by region cost **46.72s** with it and **6.09s** without; across the six dashboard cards that is 43% less work on This Month and 45% on This Year, with every figure identical. **No index repairs it**, which is why the answer is a measure set and not a migration: the sort key begins with `region_code`, which comes from `dim_region` and not from `fact_sales`, so nothing on the fact table can supply the ordering. Two other fixes were measured and rejected — collapsing the repeated year-windows into one scan came out *worse* (49.61s, because the scan was never the cost), and dropping the column from `SALES_MEASURES` outright would have silently taken the Customers page's Invoices column and a field the assistant already formats. So `DASHBOARD_SALES_MEASURES` is the variant, built like `MAP_SALES_MEASURES` with `sums` and `nullable_sums` **shared by reference** — the switch deciding what a sales report *says* is still one switch, and this one decides only what it *costs*; the tests assert that identity rather than equality, because two equal tuples today are two switches that can drift tomorrow. It is reached through `SalesMeasureToolInput.include_invoice_count`, which **defaults on** unlike every other `include_` flag on those inputs: they add a column that was never there, this one has been returned since the first sales report. The flag sits on a class between `BaseToolInput` and the three grouped-sales inputs rather than on the base, so a stock or credit tool still *refuses* it under `extra="forbid"` — a flag that quietly does nothing is a setting somebody will trust. Note that `get_target_achievement` and `get_sales_trend` never surfaced the column at all (they build their rows from named fields), so they were paying for a count nobody could read — which is also why `test_platform_api` pins this against the **emitted SQL** rather than against an absent key: a key check passes vacuously for exactly the cards that most needed checking.
